...
abstract contract DeployBase is Script {
    ...
    function _deployV1(address tempAdmin, address finalAdmin) internal returns (V1Result memory r) {
        ...
        bytes memory mgrInit = abi.encodeWithSelector(
            PUSDManager.initialize.selector, r.pusdProxy, tempAdmin
        );
    ...
    function _readWiringForV2() internal view returns (Wiring memory w) {
        ...
        try vm.envBool("UPGRADE_PUSD") returns (bool v) { w.upgradePusd = v; }
        catch { w.upgradePusd = false; }
    }
    ...
    function _revertReason(bytes memory ret, string memory fallbackMsg)
